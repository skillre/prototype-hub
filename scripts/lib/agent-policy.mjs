 ...
 * adapted for the **platform / catalog-hub** repository.
 *
 * WHAT THIS REPLACES
 * ------------------
 ...
 *
 * WHY THIS COPY IS NOT THE STARTER'S COPY
 * ---------------------------------------
 * `prototype-hub` is a **platform / catalog-hub** repository (`kind:
 * catalog-hub`), not a product derived from the Factory baseline. Two things
 * follow, and both are enforced here rather than merely documented:
 *
 *   1. Its lock must not masquerade as a product lock. `factory.lock.json`
 *      carries the honest platform shape (`kind: catalog-hub`), and the guard
 *      refuses any other kind, so the repository cannot quietly claim to be a
 *      baseline derivative it never was.
 *   2. It owns a surface the other repositories do not: the **catalog
 *      projection**. The Hub's runtime may only read
 *      `lib/generated/factory-catalog.json`; it must not reach across
 *      repositories, and when the control root is unavailable the check must
 *      say so instead of printing a pass. Those three rules are in the managed
 *      block, and `BLOCK_ANCHORS` fails if the renderer ever stops stating them.
 *
 ...
 *   factory.lock.json               what is locked   (role, versions, surfaces, unknowns)
 *
 ...
 * renderer that has quietly stopped stating a rule (see `blockAnchors`).
 */
...
/**
 * The role this repository must declare — everywhere it declares one.
 * A platform repository that calls itself a product makes every downstream
 * judgement (baseline identity, release gates, deployment ownership) wrong.
 */
export const REQUIRED_REPOSITORY_ROLE = "platform-catalog-hub"
export const REQUIRED_LOCK_KIND = "catalog-hub"

/** Commands the Hub owns in addition to the five quality gates. */
export const CATALOG_SYNC_COMMAND = "catalog:sync"
export const CATALOG_CHECK_COMMAND = "catalog:check"
export const PROJECTION_ARTIFACT = "lib/generated/factory-catalog.json"

/** The upstream reusable workflow CI would like to call — present here so one string describes it. */
...
export function blockAnchors(policy) {
  ...
  return [
    ...
    // ── 平台/目录仓专属：投影的三条规则 ───────────────────────────────────
    { id: "platform/role", pattern: /platform-catalog-hub|catalog-hub/, hint: "必须写明本仓的平台角色是 catalog-hub，而不是产品" },
    { id: "projection/read-only", pattern: /运行时只读生成物/, hint: "必须写明 Hub 运行时只读生成物、不跨仓读取" },
    { id: "projection/unknown-is-not-public", pattern: /未部署\s*\/\s*受保护/, hint: "必须写明没有已核实地址的条目显示「未部署 / 受保护」" },
    { id: "projection/upstream-honesty", pattern: /upstream[\s-]?unavailable/, hint: "必须写明上游不可用时不判 PASS" },
  ]
}
...
export function renderManagedBlock(policy) {
  ...
  return [
    ...
    "",
    `## Factory Core Policy v${policy.policyVersion}（平台/目录仓 · Agent 编排与并发）`,
    "",
    `- **仓库角色**：本仓是平台/目录仓，角色 \`${policy.repositoryRole}\` —— 它**不是**从 Factory 基线派生的产品，` +
      `锁的形状是 \`${REQUIRED_LOCK_KIND}\`（\`${LOCK_FILENAME}\`），不得伪装成 product。它拥有一个别的仓没有的面：**catalog 投影**。`,
    ...
    `- **Catalog 投影（本仓的核心不变式）**：索引由根控制面 catalog 生成（\`pnpm ${CATALOG_SYNC_COMMAND}\` / \`pnpm ${CATALOG_CHECK_COMMAND}\`，产物 \`${PROJECTION_ARTIFACT}\`）。`,
    ...
    `  - **上游不可用时不判 PASS**：找不到根 catalog 时，\`pnpm ${CATALOG_CHECK_COMMAND}\` 只验证生成物内部完整性并打印 \`[upstream-unavailable]\`，输出 \`INTEGRITY-OK (UPSTREAM UNVERIFIED)\`；它**不会**说自己通过。`,
    ...
    `- **共享路径单 owner**（\`${concurrency.sharedPaths}\`）：\`${AGENTS_FILENAME}\`、\`package.json\`、\`${POLICY_FILENAME}\`、\`${LOCK_FILENAME}\`、契约 schema、门禁脚本与 \`${PROJECTION_ARTIFACT}\` 这类共享面，同一时间只有一个 owner，其余 agent 只读。`,
    ...
    `- **部署授权**：\`${authorization.deployment}\` —— 源码发布 ≠ Production 部署。没有用户明确授权，不创建/提升 Production 部署、不改 Deployment Protection、不 push Production Branch；CI 不调用 Vercel CLI、不持有任何部署凭证。详见 \`docs/deployment.md\`。`,
    "",
    `机器可读副本：\`${POLICY_FILENAME}\` · 关键值：\`${POLICY_SCHEMA_PATH}\` · 平台锁：\`${LOCK_FILENAME}\` · 校验器：\`scripts/guard-agent-policy.mjs\`（\`pnpm factory:agents\`）。`,
    `<!-- END:${marker} -->`,
  ].join("\n")
}
...
/**
 * Does the workflow *actually run* `pnpm <command>`, as opposed to mentioning it?
 *
 * The distinction is not pedantic — it was a real false negative. The first
 * version of this check used `ciText.includes("pnpm catalog:check")`, and this
 * repository's own workflow header *explains* the catalog check in a comment.
 * Deleting the actual step therefore left the gate green: a gate satisfied by a
 * comment about the gate. Only uncommented `run:` lines count.
 */
export function ciRunsCommand(ciText, command) {
  const pattern = new RegExp(`^\\s*run:\\s*.*pnpm ${command}(\\s|$)`)
  return ciText
    .split("\n")
 ...
 * has tested. `tests/agent-policy.spec.ts` drives these with deliberately-broken
 * workflows — including a workflow whose only mention of the catalog check is a
 * comment, which is the case that used to pass.
 *
...
export function inspectCiWorkflow(ciText) {
  ...
  if (!ciRunsCommand(ciText, "factory:agents")) {
    problems.push({
      code: "ci/no-policy-gate",
      message: "CI 没有跑 `pnpm factory:agents`：策略门禁在 CI 里缺席（注释里提到不算）",
    })
  }

  // 平台/目录仓专属：投影门禁必须在 CI 里真的被执行。没有它，CI 无法回答
  // 「提交在仓里的生成物是不是仍然自洽」——而那正是本仓最容易悄悄出错的地方。
  if (!ciRunsCommand(ciText, CATALOG_CHECK_COMMAND)) {
    problems.push({
      code: "ci/no-catalog-check",
      message: `CI 没有跑 \`pnpm ${CATALOG_CHECK_COMMAND}\`：投影门禁在 CI 里缺席（注释里提到不算）`,
    })
  }
  ...
  if (!parsed.found || parsed.jobs.length === 0) {
    ...
    // Comments inside a job body do not count either — same reason as
    // `ciRunsCommand`: a step described in prose is not a step that runs.
    const runs = (job, command) =>
      job.body.some(
        (line) => !/^\s*#/.test(line) && new RegExp(`pnpm ${command}(\\s|$)`).test(line),
      )
...
export function inspectPackageWiring(pkg) {
  ...
  // 平台/目录仓专属：投影的两个命令必须真的存在，否则「索引由 catalog 生成」
  // 只是一句话，而不是一条可执行的路径。
  for (const command of [CATALOG_SYNC_COMMAND, CATALOG_CHECK_COMMAND]) {
    if (!scripts[command]) {
      problems.push({
        code: "package/missing-catalog-script",
        message: `package.json 没有 \`${command}\` 脚本——投影工作流没有被接上`,
      })
    }
  }
  if (
    scripts[CATALOG_CHECK_COMMAND] &&
    !String(scripts[CATALOG_CHECK_COMMAND]).includes("scripts/sync-factory-catalog.mjs")
  ) {
    problems.push({
      code: "package/catalog-script-target",
      message: `\`${CATALOG_CHECK_COMMAND}\` 没有指向 scripts/sync-factory-catalog.mjs：${scripts[CATALOG_CHECK_COMMAND]}`,
    })
  }
...
export function formatPolicyReport(report) {
  const lines = []
  lines.push(
    `  策略 v${report.policyVersion} · 平台仓 ${report.repositoryRole} · hub v${report.hubVersion} · 锁形状 ${report.lockKind} · 受管面 ${report.managedSurfaces} 个`,
  )
